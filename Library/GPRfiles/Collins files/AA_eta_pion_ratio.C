...
Int_t AA_eta_pion_ratio(const Int_t data_number, Double_t input_pt[], Double_t input_EtaPionRatio[], Double_t input_EtaPionRatioErr[])
{
  ifstream filein;
  string trashline;
  int output=0;
  ...
  double AA_pt[nPred],AA_y[nPred],AA_ey[nPred];
  filein.open("AA_maximal_range_smooth.txt");
  int header_lines=2;
  ...
  {
    filein>>AA_pt[i]>>AA_y[i]>>AA_ey[i];
    if(output==1)cout<<"i="<<i<<", (x,y,ey)=("<<AA_pt[i]<<","<<AA_y[i]<<","<<AA_ey[i]<<")"<<endl;
  }
  filein.close();
  ...
  {
    input_EtaPionRatio[i]=continuity(input_pt[i],AA_y);
    input_EtaPionRatioErr[i]=continuity(input_pt[i],AA_ey);
